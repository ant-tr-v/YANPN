...
model = Sequential()
model.add(Conv2D(net_weights[0], net_weights[1], padding=1))
model.add(ReLU())
# model.add(MaxPool2D(2))
model.add(Conv2D(net_weights[2], net_weights[3]))
model.add(ReLU())
model.add(BatchNorm2D(net_weights[4], net_weights[5], net_weights[8], net_weights[9]))
...
y_test = keras.utils.to_categorical(y_test, num_classes)[:5000]
